...
class Model_Cart extends Model_Core_Row
{
    ...
    public function getBillingAddress($reload = false)
    {
        ...
        $address = $addressModel->fetchRow("SELECT * FROM `cart_address` WHERE `cartId` = {$this->cartId} AND `billingAddress` = 1");
    ...
    public function getShippingAddress($reload = false)
    {
        ...
        $address = $addressModel->fetchRow("SELECT * FROM `cart_address` WHERE `cartId` = {$this->cartId} AND `shipingAddress` = 1");
        if(!$address)
        {
            return $addressModel;
        }
