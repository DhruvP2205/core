...
class Model_Order extends Model_Core_Row
{
    ...
    public function getBillingAddress($reload = false)
    {
        ...
        $address = $addressModel->fetchRow("SELECT * FROM `order_address` WHERE `orderId` = {$this->orderId} AND `billingAddress` = 1");
        if(!$address)
        {
            return $addressModel;
        }
    ...
    public function getShippingAddress($reload = false)
    {
        ...
        $address=$addressModel->fetchRow("SELECT * FROM `order_address` WHERE `orderId` = {$this->orderId} AND `shipingAddress` = 1");
        if(!$address)
        {
            return $addressModel;
        }
    ...
    public function getComment($reload = false)
    {
        ...
        $comment = $commentModel->fetchRow("SELECT * FROM `comment` WHERE `orderId` = {$this->orderId}");
        if(!$comment)
        {
            return $commentModel;
        }
